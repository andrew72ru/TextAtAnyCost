...
final class Ppt extends Cfb
{
    public function parse(): ?string
    {
        ...
        for ($j = 0, $jMax = count($offsetPersistDirectory); $j < $jMax; ++$j) {
            ...
            for ($k = 0, $kMax = strlen($rgPersistDirEntry); $k < $kMax;) {
        ...
        $out = '';
        for ($i = 0, $iMax = strlen($slideList); $i < $iMax;) {
                    ...
                    while (preg_match("#(\xA8|\xA0)\x0F#", $drawing, $pocket, PREG_OFFSET_CAPTURE, $from)) {
                        ...
                        if (substr($drawing, $pocket[1] - 2, 2) === "\x00\x00") {
                            ...
                            if (ord($pocket[0]) == 0xA8) {
                                ...
                                $out .= $this->unicodeToUtf8($this->getRecord($drawing, $pocket[1] - 2, 0x0FA0)) . ' ';
                            }
                        }
    ...
    private function getRecordLength($stream, $offset, $recType = null)
    {
        ...
        if ($recType !== null && $recType !== $this->getShort(2, $rh)) {
            return false;
        }
