 ...
 * Class Pdf.
 */
final class Pdf implements ConverterInterface, ReadFileInterface
{
    /**
     * @var false|string
     */
    ...
    protected function decodeAsciiHex(string $input): ?string
    {
        $output = null;
    ...
    protected function decodeAscii85(string $input): ?string
    {
        $output = null;
    ...
    protected function getObjectOptions(string $object): array
    {
    ...
    protected function getDecodedStream(string $stream, array $options): ?string
    {
        ...
        if (empty($options['Filter'])) {
            ...
            foreach ($options as $key => $value) {
                ...
                if ($key === 'FlateDecode') {
                    $_stream = $this->decodeFlat($_stream);
                }
            }
            $data = $_stream;
        }
    ...
    protected function getDirtyTexts(&$texts, $textContainers): void
    {
    ...
    protected function getCharTransformations(&$transformations, $stream)
    {
        ...
        for ($j = 0, $jMax = count($chars); $j < $jMax; ++$j) {
        ...
        for ($j = 0, $jMax = count($ranges); $j < $jMax; ++$j) {
    ...
    protected function getTextUsingTransformations($texts, $transformations): ?string
    {
                ...
                switch ($c) {
                    ...
                    case '\\':
                        ...
                        elseif ($c2 >= '0' && $c2 <= '9') {
                            ...
                            $oct = preg_replace('#[^0-9]#', '', substr($iValue, $j + 1, 3));
